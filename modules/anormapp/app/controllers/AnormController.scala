...
import controllers.anormapp.AnormForm.{AnormData, anormForm, anormParser, attrForm}
...
class AnormController @Inject()(db: Database, cc: MessagesControllerComponents)
  ...
  def add() = Action {implicit request =>
    // フォームにidを指定できないことから、anormFormではなくattrFormを指定する
  ...
  def create() = Action {implicit request =>
    ...
    Redirect(controllers.anormapp.routes.AnormController.index())
  }
